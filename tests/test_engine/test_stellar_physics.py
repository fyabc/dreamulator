...
class TestSolarDay:
    """Solar (synodic) day: 1/P_solar = |1/P_sidereal − 1/P_year|."""

    def test_earth(self):
        """Earth: sidereal 1 d, year 365.25 d → solar day ≈ 1.0027 d."""
        P = solar_day_days(1.0, 365.25)
    ...
    def test_moon_synodic_month(self):
        """The Moon: 27.32 d sidereal month, 365.25 d year → 29.53 d."""
        P = solar_day_days(27.32, 365.25)
        assert pytest.approx(29.53, rel=1e-2) == P

    def test_mercury_3_2_resonance(self):
        """Mercury: 58.65 d rotation, 88 d year → 176 d solar day."""
        P = solar_day_days(58.65, 88.0)
        assert pytest.approx(176.0, rel=0.01) == P

    def test_star_locked_body_has_infinite_solar_day(self):
        """P_sidereal == P_year (tidally locked to the star) → None."""
        assert solar_day_days(16.6, 16.6) is None

    def test_slow_prograde_rotator(self):
        """Rotation slower than the year: frequency difference still positive."""
        P = solar_day_days(100.0, 50.0)
        assert pytest.approx(100.0, rel=1e-9) == P  # |1/100 − 1/50| = 1/100


class TestPolarGeometry:
    def test_polar_circle_earth(self):
    ...
    def test_zero_tilt_no_polar_day(self):
        assert polar_circle_latitude_deg(0.0) == pytest.approx(90.0)
        assert polar_day_fraction_of_year(0.0, 90.0) == 0.0

    def test_pole_has_half_year_day(self):
        assert polar_day_fraction_of_year(23.44, 90.0) == pytest.approx(0.5)
        assert polar_day_fraction_of_year(9.0, -90.0) == pytest.approx(0.5)

    def test_polar_circle_boundary_is_zero(self):
        assert polar_day_fraction_of_year(23.44, 66.56) == pytest.approx(0.0, abs=1e-6)

    def test_below_polar_circle_is_zero(self):
        assert polar_day_fraction_of_year(23.44, 45.0) == 0.0

    def test_north_cape_matches_observation(self):
        """71.2°N (North Cape): ≈0.204 of the year ≈ 74 geometric days."""
        frac = polar_day_fraction_of_year(23.44, 71.2)
        assert frac == pytest.approx(0.204, abs=0.01)
        assert frac * 365.25 == pytest.approx(74.4, abs=1.0)
