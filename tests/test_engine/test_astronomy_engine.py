...
def _write_planets_yaml(world_dir: Path, planet_id: str, orbits: str) -> None:
    """Write a minimal planets.yaml into the geological input layer."""
...
def earth_like_world(tmp_path: Path) -> Path:
    ...
    _write_planets_yaml(world_dir, "planet_test", "star_test")
    return world_dir
...
def luminosity_only_world(tmp_path: Path) -> Path:
    ...
    _write_planets_yaml(world_dir, "planet_lum", "star_lum")
    return world_dir
...
def inconsistent_world(tmp_path: Path) -> Path:
    ...
    _write_planets_yaml(world_dir, "planet_inc", "star_inc")
    return world_dir
...
def _make_engine(world_dir: Path) -> AstronomyEngine:
    ...
    if geo_dir.exists():
        layer_input_dirs["geological"] = geo_dir
...
class TestAstronomyEngine:
    ...
    def test_system_catalog_written(self, earth_like_world: Path):
        """system_catalog.yaml merges stellar.yaml + planets.yaml per body."""
        engine = _make_engine(earth_like_world)
        result = engine.run()
        assert result.success

        sc_path = earth_like_world / "layers" / "astronomy" / "derived" / "system_catalog.yaml"
        ...
        (star,) = sc["stars"]
        assert star["id"] == "star_test"
        assert star["luminosity_sol"] == pytest.approx(1.0, rel=0.01)
        assert "habitable_zone" in star

        (body,) = sc["bodies"]
        assert body["id"] == "planet_test"
        assert body["in_planets_yaml"] is True
        assert body["orbit"]["period_days"] == pytest.approx(365.25, rel=1e-3)
        assert body["derived"]["solar_day_days"] == pytest.approx(1.0, rel=1e-2)
        assert body["derived"]["in_conservative_habitable_zone"] is True
