...
def gaia_m_cells():
    ...
    try:
        mesh = json.loads(
            __import__("pathlib").Path(mesh_path).read_text(encoding="utf-8"),
        )
    except FileNotFoundError:
        pytest.skip("gaia-m mesh not found — run 'dreamulator build gaia-m' first")
    return mesh["cells"]
...
def test_tropical_band_dominated_by_tropical_biomes(gaia_m_cells):
    ...
    land_cells = [
        ...
        and abs(c["lat"]) <= 15
        and c.get("biome")
        is not WhittakerBiome.OCEAN.value
    ]
    ...
    if tropical_count < temperate_count:
        ...
        warnings.warn(
            ...
            f"planet like gaia-m (land mean T {3.7}°C). Climate tuning (3A.4) "
...
def test_mid_latitude_dominated_by_temperate_biomes(gaia_m_cells):
    ...
    assert dom in ("temperate", "ocean"), (
        f"Mid-latitude (30-50°) expected temperate-dominated, got {dom}. "
        f"Counts: {biome_counts.most_common(8)}"
    )
...
def test_npp_latitude_gradient(gaia_m_cells):
    ...
    land_cells = [
        ...
        for c in gaia_m_cells
        if c.get("crust_type") == "continental"
        and c.get("npp_gc_m2_yr") is not None
    ]
    ...
    # Compute means
    means = {
        band: sum(vals) / len(vals) for band, vals in band_npps.items() if vals
    }
