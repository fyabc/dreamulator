...
class TestSeasonalTemperature:
    ...
    def test_no_tilt_no_season(self) -> None:
        """Zero axial tilt → no seasonal variation."""
        ...
        assert np.allclose(result["jan"], result["jul"], atol=1e-3)

    def test_poles_largest_range(self) -> None:
        """Polar regions have the largest annual range."""
        ...
        # Polar annual range >> equatorial
        assert result["annual_range"][1] > result["annual_range"][0]

    def test_opposite_seasons_nh_sh(self) -> None:
        ...
        # NH summer: 45°N > 45°S
        assert result["today"][0] > result["today"][1]
