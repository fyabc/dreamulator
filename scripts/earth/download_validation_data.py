...
Usage::
    ...
    uv run python scripts/earth/download_validation_data.py            # download all
    uv run python scripts/earth/download_validation_data.py --dry-run   # show pending downloads
    uv run python scripts/earth/download_validation_data.py --skip-soda # skip slow SODA stream

After download, the following commands can run offline:

    uv run python scripts/earth/import_earth_elevation.py --skip-download
    uv run python scripts/climate/convert_koppen_map.py --tif <cached_zip>
...
def main() -> None:
    ...
    #    when absent).
    soda_nc = Path(__file__).resolve().parents[2] / _SODA_DEST_RELPATH
    if skip_soda:
    ...
    if dry_run:
        ...
        print("  uv run python scripts/earth/import_earth_elevation.py --skip-download")
        print("  uv run python scripts/climate/convert_koppen_map.py")
        print("  uv run python scripts/earth/import_earth_climate.py")
