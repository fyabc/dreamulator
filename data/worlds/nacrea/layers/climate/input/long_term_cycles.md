...
| 3 | 气候岁差（Aegis 自转轴自由进动主导）→ 半球季节增强/减弱 | **~550 yr**（气候岁差实测 540–560，发布带 390–640） | 近日点对齐某半球夏 → 该半球季节增强、对半球减弱（距离季 ±3% × 倾角季相干叠加） | 未建模；季节 EBM 用 epoch 相位 ϖ=0；**叙事锚点：千年尺度气候纪元交替** |
| 4 | 行星链长期耦合 → 偏心率调制 | **~45 kyr（1 Myr N 体积分确认）** | Aegis e 0.016–0.062 摆动 → 季节日照对比 ±3–12% | 未建模；**类米兰科维奇冰期旋回（地球 41 kyr 倾角旋回同量级）** |
| 5 | 潮汐加热振荡 ⭐**卫星特有**「万年活动周期」 | **11 kyr**（韵珠/守珠长期摄动拍频） | e_m 受迫带 0.0008–0.009（A4v 认证带，历元 0.0018；硬度豁免见 design-notes/0009）→ 地质热通量 0.05–6 W/m²（均值 ~0.9） | 未建模；经火山-CO₂ 通路影响气候；**设定特色节律** |
| 6 | 长期摄动与系统稳定性 | 10⁴–10⁶ yr | Sentinel（e=0.18, i=7°）远距摄动；韵珠/守珠均单体准可积（径向净空 {{ entities.satellite_cadence.mutual_hill_separation_to_outer | round0 }} r_H,m，Gyr 级，REBOUND 三层认证） | 未建模；生态/宜居窗口时标 |
| 7 | 潮汐演化长期漂移 | Gyr | 卫星外迁（已形成以来 +10%）；加热衰减 → 火山 3×⊕ 递减 → CO₂ 源衰减 | 未建模；碳-硅酸盐循环长期平衡 |
| 8 | 恒星演化 | Myr–Gyr | Ignis（K8）前主序阶段曾亮数倍（~1 Gyr）；主序缓慢增亮 | 天文层已有恒星演化模型（evolution_progress={{ entities.star_ignis.evolution_progress | round2 }}）；未耦合气候 |
...
## 3. 气候岁差 → 半球季节增强/减弱（~550 yr）

Aegis 自转轴自由进动 ~550 yr（J₂ 带 0.006–0.010 内 490–816 yr）比近日点进动
~24–60 kyr 快两个量级，主导「近日点相对至日」的相位扫掠——气候岁差拍频
**~540–560 yr**（发布带 390–640 yr，orbital_dynamics.md）。

- 机制：距离季（e=0.03 → 全球 ±3%，近日点在 Aegis 轨道上固定）与倾角季（9°，南北反对称）
  同频叠加；近日点对齐某半球夏至 → 该半球季节增强、对半球减弱
  （`T_amp^N/S = √(T_obl² + T_dist² ∓ 2·T_obl·T_dist·sinΔ)`，Δ = 近日点−至日相位）。
- **叙事锚点**：每 ~550 yr 扫一轮，「北夏强/南夏强」交替——千年尺度气候纪元交替，
  与 §4 的 45 kyr e 旋回（冰期/间冰期）构成多频气候节拍。当前 epoch ϖ=0 落在对称相位
  （近日点对齐春分、无半球不对称），只是岁差周期里的一个快照，非永久构型。
- 引擎状态：季节 EBM 用 epoch 相位 ϖ=0（静态），未建模时变岁差；表达「纪元」只需改 Aegis
  `argument_of_periapsis`（纯相位平移、无新物理）。

## 4. 行星链长期振荡（~45 kyr，数值积分确认）

Aegis–Boreal–Glacis 三星处于 **1:2:4 周期通约链**（半长轴比率冻结，会聚迁移
产物）。拉普拉斯角 φ_L 经 1 Myr N 体积分（2 kyr 细采样）实测为**循环而非
天平动**（~2–8 圈/Myr，速率依历元相位）——链是「共振邻接」而非相位锁定；
近共振的长期摄动耦合仍持续交换偏心率与近日点经度，实测节律不受影响：
...
0.0008–0.009 受迫带（A4v 认证带，rms 0.0035；线性标定预言 e_rms≈0.0026；
历元值 0.0018 为带下缘快照，机制与硬度豁免详见 design-notes/0009）。这正是
Io 机制（Io e=0.0041 → 太阳系火山最活跃天体）。潮汐耗散 ∝ e²：

- 地质热通量在 0.05–6 W/m² 带内振荡（均值 ~0.9 W/m²），火山活动
  （当前设定 3×⊕）随之波动，主调制周期 **11 kyr（「万年活动周期」，设定
  特色节律**：火山-地热-CO₂ 的脉冲呼吸，与 §4 的 45 kyr 日照旋回构成
  双频气候节拍）
- 气候通路：火山 CO₂ 释放率振荡 → 大气温室浓度振荡 → 冰期/暖期旋回
  （与 §4 的 45 kyr 日照旋回叠加，构成双频气候节拍）
...
## 6. 长期摄动与系统稳定性（10⁴–10⁶ yr）

- Sentinel（3.68 AU, e=0.18, i=7°）作为远距散射体提供长期摄动
- 行星共振链 1 Myr 积分稳定；韵珠、守珠为单体远距逆行轨道（径向净空
  {{ entities.satellite_cadence.mutual_hill_separation_to_outer | round0 }} r_H,m、恒星潮汐仅有界摆动）；其 >25 kyr 长期动力学按硬度豁免处理
  （判决与豁免记录见 design-notes/0009 §2、附录 §6.7）
- Aegis e 实测峰值 0.062：§4 的 45 kyr 冰期旋回是**进行中的现实**，
