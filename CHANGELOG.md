...
- **§5-β 对流临界雨出门（2026-09-17，供给端沙漠 P 攻关；验收裁决 = W 场供给对比是
  硬前置，默认关）**。诊断先行（三轮构建）：观测的雨出效率 τ=W/P 在深撒哈拉约
  150 天、对流区 7-10 天，相差 15-20 倍，而引擎全局均匀 1/9 天——沙漠柱水汽量级
  正确却被每九天雨出一遍（撒哈拉 343 vs obs 30），过境洋面的水汽也在传输中按
  e^(−t/τ) 漏掉（孟加拉湾 W 29.5 vs 赤道印度洋 53.8，恒河陆地被饿死的 W 侧根源）。
  实现为 `climate_physics.convective_pickup_gate`：k_rain 乘 f(W/W_sat)（W_sat 与
  冷阱同函数，冷区自动豁免、暖池不动），物理锚 Neelin-Peters-Hales 2009 的降水
  临界抬升（β≈0.23、95% 降水在 0.8w_c 上）与 Schiro 2016/Ahmed 2017 的陆地临界
  偏低；陆洋全域统一处理（过境洋面少漏 → W 回升 → 季风陆地基过临界 → 门开的
  自释放回路）；iterate-twice 求解保持线性（门在 W₀、W₁ 各算一次——单次迭代会
  把门钉死在饿死的 W₀ 上，首轮验收实测恒河掉到 0.35×）。earth 两轮全构建验收：
  门按设计工作（沙漠盒全降 343→198、不动盒 1.00-1.12 完美、自释放方向正确），但
  全球陆格逐格 P R² 0.296→0.238 触发预注册中止判据（−0.058 > 0.03）——收敛不动点
  把每柱 P 变成其水汽输入，同 x 档内沙漠与恒河/华南所需门因子相差一个量级（共位
  方差，结点无解）。裁决 = `convective_pickup_gate_enabled` 默认关、机器全部保留；
  **W 场供给对比（恒河 |Q| 9×、过境漏湿、撒哈拉输入通道）确认为 §5-β/④ v2/§5-α
  三门的共同硬前置**。标定模式 `diagnose_desert_wetness --pickup-gate` 与知识库
  条目（precipitation.md，临界雨出物理 + 自释放）一并入库。

- **④ v2 两层 Gill 定常波（2026-09-17，Lee-Wang-Mapes 2009 两模态模型；求解器
  全验证，消费门标定无信号默认关）**：`map/stationary_wave_two_level.py`——正压 ψ +

- **④ v2 两层 Gill 定常波响应（2026-09-17，Lee-Wang-Mapes 2009 两模态模型；求解器
  全验证，消费门标定无信号默认关）**：`map/stationary_wave_two_level.py`——正压 ψ +
  斜压 ψ̂/χ̂/φ̂ 四场定常线性系统（论文 eq 11/13/15/17，参数 c_g=60 m/s、γ=(2d)⁻¹、
  r₁=(10d)⁻¹、r₀=(20d)⁻¹、A=1e6 m²/s 全论文值）；基本态 = 纬向平均（表面纬向风 +
  热成风剪切 ±40 m/s 帽、高纬 taper 55°→75°）——v1 病灶（局域 bin 噪声/地形污染/
  非地转 v̄）结构性消失；**k-块对角求解**（zonal 基本态 ⇒ 经度 rFFT 后按 wavenumber
  块对角，交织排列 + `solve_banded` 带宽 11，**0.16 s/月** vs v1 全 2D 稀疏 LU 35 s，
  本机复数稠密 zgesv 单次 126 ms 的慢 BLAS 下带状是唯一可行路径）；消费 = 中层
  w = −p_m∇²χ̂/(ρ_m·g) → 陆地 k_rain 下沉干燥门 `subsidence_rainout_gate`（质量守恒
  乘性调制，与 storm/SST 门在 `omega_gate_monthly` 钩子复合；**风场保持 pass-1**——
  v1 的 ΔSLP→BL 风 1/f 放大伤害路径切断）；config `stationary_wave_v2_enabled`
  （默认关，与 v1 flag 互斥 fail-loud）。**求解器验证**（14 项单测）：静止基本态 ψ
  严格为零（论文 Case-1 精确解耦 = 纯 Matsuno-Gill）、赤道源南北正压反气旋对（论文
  Case-4 原文数值版）、理想化印度源 ω 的 R&H 签名（撒哈拉 +3.5e-3/中东 +5.2e-3/
  地中海 +2.1e-4 Pa/s 下沉）在 rest/Case-4/现实急流三基本态不变号、Kelvin 东传单调
  衰减、慢自转波导 Ω^(-1/2) 加宽（52°→80°）、无自转涡旋响应消失、阻尼×0.5/×2
  鲁棒、月度叠加原理。**标定轮结论（`diagnose_desert_wetness --wave-gate` 新模式，
  存档驱动免重跑引擎）：信号弱且非单调**（GPCP 陆格 e_rel 上升 bin 0.95 vs 深下沉
  bin 0.84-1.04，n≈2e4）——根因 = **强迫自引用**：模型沙漠湿偏差作为虚假加热进入 Q
  （7 月撒哈拉模型 P 58 mm ≈ 恒河 56 mm），淹没 R&H 遥相关下沉（撒哈拉 7 月模型 w
  反为上升 +6e-4；华南年尺度误下沉而 obs e_rel 2.76 已欠雨）。结点取保守弱斜率
  (0,1.0)→(−1e-3,0.93)→(−3e-3,0.85)；**解锁前置 = 供给端沙漠 P 偏差修复**，届时
  重标定（标定模式已就位）。教训：要用波修的湿偏差正是污染波强迫的东西——消费端
  机制（§5-α 门/④ ω 门）连续两轮被输入场保真度阻塞，供给端（水分路由/沙漠 P）
  成为两个门的共同前置。
...
- **季风 ΔP 海拔结构（季风轮 B1，§2 底稿落地）**：`ΔP = −P(z)·f(z)·ΔT/T̄_zonal`，
  P(z)=exp(−z/8.5km) 气柱质量 + f(z)=f_deep·exp(−z/3km) 水汽顶衰减（Wu 2012：85% 水汽
  <3km；Boos & Kuang 2010/2013：热源=低地非抬升加热）——青藏 4844m 表面异常主导 ΔP 的
  伪影移除（4.8km 格响应 ×0.113），印度低地热低压接管。f_deep = `monsoon_depth_fraction`
  （config，默认 0.25 低地不变，Stage C 标定靶）。效果：India 263→353mm + 最湿月
  Oct→Aug、华南 274→297、撒哈拉 539→390、Sahel 987→879、刚果 Af 375/375；逐格 P R²
  0.259→0.298（超轮前基线 0.292）、5 群 kappa 0.538、Köppen 分布 70.7%。暴露 → B2
  候选：低压中心落卡拉库姆 40°N（全纬向参照在热沙漠带自稀释）而非伊朗/塔尔 30°N，
  参照系改纯海洋纬向均值待实验；索马里 +670→+830（下沉干燥缺失连带，§5）。
- **季风 ΔP 参照系改纯海洋纬向均值（季风轮 B2，用户裁决采纳）**：`zonal_mean_monthly`
  加 `mask` 参数（调用传 `ocean_mask=is_ocean`）——ΔT = 陆格 vs 同纬海洋（docstring
  「海陆热力对比」本义；全纬向参照在 25-35°N 热沙漠带自稀释，低压中心被推到卡拉库姆
  40°N）；全陆地世界回退全格均值（修 end-member 测试 argmin 崩溃）。判据实测：印度
  低地 ΔP −3.67→−5.66 hPa（进观测区间 −5~−7）、恒河 301→492mm（收敛率 38%）、孟加拉
  244→370、India 首现 Cwa 31 格、爪哇 −550→−240、纬向 10..19 带 −151→−4、逐格 P R²
  0.298→**0.359（本轮最大单步）**、bias −211→−123、RMSE 585→559、T 持平。代价 =
  撒哈拉 390→660（obs 30）、索马里 +953——判定为 §5 下沉干燥缺失的揭蔽（真实撒哈拉
  7 月确有热低压，不下雨因下沉逆温盖；引擎把正确低压变错误降雨），§5 下沉干燥提为
  本轮下一子项；θeb 参照精化（Hurley & Boos 2013：θeb 异常湿度主导，需 Stage 2
  湿度场）登记 steady-coupling 族候选。
- **季风 ΔP 振幅标定 = Stage C ✅（诊断结论反转：形状/结构，非全局振幅）**：离线对标
  NCEP SLP 季节距平（新 `diagnose_monsoon_dp_shape.py`，主口径 D = 陆地减同纬海洋对比
  距平，与 `pressure_anomaly_monthly` 构造逐项对应；旁证 A = 陆地 only）。**pre-flight
  月份索引 bug 实锤**：原「干内陆过强（哈萨克 2.84×/西伯利亚 3.91×）→ 两趟 g(W) 阻尼」
  的结论，是把模型月份索引（month 0 = 三月，Jul=4/Jan=10）喂进 NCEP（month 0 = 一月）
  ——实际读了 5 月/11 月肩季，距平约半，制造了假形状。修正后**季风 ΔP 已标定**：

  | 框（月） | 模型 ΔP | NCEP-D | ratioD | NCEP-A | ratioA |
  |---|---|---|---|---|---|
  | 印度低压（7） | −5.93 | −5.95 | **1.00** | −8.18 | 0.73 |
  | 哈萨克/图兰（7） | −11.85 | −9.16 | **1.29** | −11.52 | 1.03 |
  | 华南（7） | −4.69 | −6.42 | **0.73** | −8.50 | 0.55 |
  | 撒哈拉（7） | −7.04 | −1.41 | **5.01** | −3.47 | 2.03 |
  | 西伯利亚（1） | +18.71 | +7.66 | **2.44** | +9.08 | 2.06 |
  | 蒙古（1） | +10.17 | +13.08 | **0.78** | +14.95 | 0.68 |

  6 框 4 落 0.73–1.29（印度/哈萨克/华南/蒙古）= B1/B2 标定成功。**三候选区域重加权
  全证伪**：① g(W) 湿度阻尼——西伯利亚（W≈1.5）需 ×0.41、蒙古（W≈1.4）需 ×1.29，
  同 W 反向修正、单调 g(W) 不可分；模型撒哈拉 W 被湿偏置抬高（40.8）反而欠阻尼最坏框。
  ② aridity-keep 阻尼——模型 keep 被季风干偏置抬高（印度 0.88/华南 1.00），按 keep 阻尼
  压垮已标定季风低压（印度 1.00→0.15）；g(W)/keep 都 gate 在被研究降水偏置污染的场上=循环。
  ③ `_dt_subsidence` 作非循环 ΔP 帽信号——前置验证：它 = 1D EBM 纬向 T(lat) × 纬向哈得来
  下沉权重 = **纬向对称**，撒哈拉/印度/华南同纬度取值几乎相同，分不开。**真残差归因**：
  撒哈拉 7 月 5×（过深）+ 西伯利亚 1 月 2.4×（过强）= 模型**缺副热带反气旋**（只在热陆
  加工低压、无高压侧；真实撒哈拉 SLP = 热低压被副高部分抵消），属结构缺口（roadmap ④
  静止波/SLP 逐格），非振幅标定可收。季风降水仍过干（恒河 492/1347）但 ΔP 已标定 →
  缺口在**降水侧**（水分路由/辐合增强 → Stage F）。**月份口径教训**：模型 0=三月、
  NCEP/obs 0=一月，对标须 `(model_m+2)%12` 映射（其余月度验证脚本均已正确处理，唯
  pre-flight 一次性代码踩坑）。
- **§5 下沉干燥诊断裁决（2026-09-13，零引擎改动）**：新 `diagnose_desert_wetness.py`
  双模式（秒级存档标定 + 三消融归因 A 基线/B 季风关/C κ 关）把「热沙漠过湿族」分解为
  **三族不同根**：① **A3 内陆**（撒哈拉腹地 565/obs 27、萨赫勒 872/441、澳洲沙漠
  1132/326、卡拉哈里 1047/441）——供给过决定（季风热低压平流 40-64% + κ 扩散 35-59%
  + Budyko 再循环 ET 248 mm；无季风时撒哈拉仍 308），根治 = ④ 副热带反气旋
  （Rodwell & Hoskins 2001 季风强迫 Rossby 下沉、沙漠与季风同枚硬币两面；Gill 1980
  线性斜压响应 = 轻量候选实现）；② **A1 暖岸**（波斯湾沿岸 352/151、红海西岸
  348/91）同归 ④；③ **A2 寒流岸 + B 赤道干带**（纳米布 755/216、阿塔卡马 323/88、
  加拉帕戈斯 +2611、索马里/肯尼亚岸 +957~+2226、东太平冷舌 +1950 = 全球 TOP 湿偏差
  格）——根 = §4 **SST 距平结构死亡**（归档 `sst_anomaly_c` std 0.17 °C、索马里/
  加拉帕戈斯/秘鲁/加那利/本格拉盒 |ΔSST|<0.3、热带 SST 顶满 28.0 °C 暖池上限；真实
  寒流 −4~−8 °C）。**纬向对称杠杆全线量化证伪**：恒河 κ 依赖 78%（C/A 0.22）、华南
  65%、季风占恒河 49%——均 ≥ 撒哈拉（59%/46%），任何纬向 k_rain 帽/κ 削减/W 帽打
  验收（恒河 −650、华南 −1236 已偏干）比打沙漠更狠；RH 判据被 T 场污染反向（7 月
  撒哈拉 30.5 °C < 恒河 34.9 °C，与实况倒置 → 门方向打反）。**§5-α SST 对流门已标定
  待 §4**：f(ΔSST) 经验曲线（GPCP 海洋格：热带 e_rel 0.94@0 → 0.60@−2 °C、外热带
  1.00@0 → 0.65@−5 → 0.24@−7；WTG Sobel 2001 + SST 阈值族 Johnson & Xie 2010 +
  信风逆温 Klein & Hartzenius 1997），k_rain 乘法调制质量守恒、仅负距平触发、暖流
  零抑制（孟加拉湾 +0.07/南海 +0.09/黑潮 +0.21）= 恒河/华南供水结构性安全。**F 线
  新锚点**（消融副产物）：西高止季风贡献 ≈0（B/A 1.02）、阿拉伯海海上雨出 795 vs
  obs 356（水汽到岸前掉完）、几内亚湾去季风反而更湿（1073→1991，obs 1147）= 西非
  水分被撒哈拉热低压（ΔP 5× 过深）错误北抽（几内亚湾去季风反而更湿 1410→2468）。
  **顺序修正**：§5 无独立可实施项（A3/A1
  等 ④、A2/B 等 §4），捆绑轮下一可实施子项 = D/F 水分路由。proposal §4/§5/§7 +
  roadmap P0 季风行同步。

### Added

- **草案七（四氢化萘密度翻转油海）吸收外部讨论 + 核查修正（2026-09-15，chat-碳富集
  行星的油海形成机制）**：吸收四组内容——① **「过冷水」物理修正**：宏观水体无法维持
  深度过冷，下层水被界面冰潜热钳制在 ≈0 °C 恒温（ρ≈1.000），密度交叉由油温单边驱动，
  交叉点 −12 → **−15 °C**（按草案自身膨胀公式复算 ρ_oil(−15)=0.9999≈1.000 吻合），设定
  反而更稳固；② **全球分布重构「赤道为膜、极地为海」**：油透镜体厚度上限 = 毛细长度
  ℓ=√(γ/(g·Δρ))，低纬 Δρ≈0.034 → ℓ≈1–2 cm 全球薄油膜（体积 <0.1%、构成全球光化学
  消耗面），极地 Δρ→0 中性浮力解除束缚 → 数十米深透镜体；源-汇-传送带（低纬净耗/
  极地净积累/风生流+冰盖传送带）+ 厚度差异五来源（极地涡旋锁定/海盆地形/雾霾光学
  厚度/盐度断层/热液超供）；③ **生化补全**：脂溶性铁载体（跨油水界面 Fe³⁺ 穿梭）+
  十氢萘 = LOHC「副油箱」（4.3 wt%，41 g H₂ 需 ~940 g，接入既有氧化还原链）；④
  **地质事件级体积波动**（温室失控光解萎缩 / LIP 热裂解反弹 / 撞击冬天冻结休克 /
  裂解蛇纹石化脉冲 / 生物乳化埋藏泵 / 倾角漂移翻转停摆）+ 稳态界面净化机制 + 冬季
  「白冰封油」双层冰壳奇观。**核查否证/修正的 chat 说法**：「低纬热液油因密度大形成
  深海冷泉油湖」方向反了（四氢化萘 30 °C 密度 0.96 < 水，轻馏分上浮并入薄膜，仅重质
  焦油滞留/下沉）；「数百米厚极地油海」与总油量 = 水海 1/470 的体积预算不符（8% 表
  面积 × 地球级水海 → 平均 ~50 m，改「数十米、海盆中心上百米」）；透镜体厚度公式
  h∝1/√(gΔρ) 漏了界面张力 γ，已改为完整毛细长度标度；「停滞盖构造」建议与草案的
  俯冲回收设计冲突，未采纳。chat 第一部分（C/O>1 碳行星 + 晚期薄层输水 + 生物脂海）
  属草案六（富碳辛烷油海）范畴，本轮未动。
...
- **`docs/design/proposals/unified-climate-taxonomy.md`**（2026-09-14 设计轮 +
  ...
  锚点法七类（3.1 溶剂液窗 θ 归一化温度轴——**Köppen 魔数 {−3,0,10,18}°C = 地球水窗
  （0–100 K）θ 节点 {−0.03,0,0.10,0.18}**，地球档案零覆写按构造重现 Köppen；θ 可移植
  性 sanity 锚 = 泰坦甲烷窗 0.13 ≈ 地球 0.15；生物圈接口并入本节、bio_anchors 默认值
  = 这组 θ 节点、DAG 依赖方向问题入 7.3。3.2 UNEP AI=P/PET 水分轴 + Hamon PET 定义，
  复用 subsidence_aridity_gate 既有链路。3.4 广义季节 = 振幅/生长季（Trewartha 吸收）/
  集中度/T-P 相位（Ps/Pw/Pf/P2，跟随主流 s/w/f）正交拆段，修 Köppen s/w 双重含义歧义。3.5 环流体制
  只保留世界级 C&lt;n&gt;/TL，cell 级环流带移除——与热量带加水分带冗余、单圈退化，登记
  已否证）+ 分段编码语法（只 / 与 - 两种分隔符、ASCIIPA 式可扩展：未知段透传 +
  profile rebind）+ 世界档案 YAML（溶剂表/归并/别名 → Köppen）+ 覆盖矩阵（世界清单
  逐项判定）+ 实施分期 Phase 1-3（Phase 1 纯函数零新 input，验收 = UCC→Köppen 别名
  逐 cell 一致率 ≥95%）。调研缺口：「逐 cell × 跨溶剂 × 跨自转」统一分类无学术先验
  （整行星 Kopparapu/PHL、锁定 Turbet/terminator、泰坦 Olcott 2024、社区 Pasta
  Bioclimate 各占一角）→ 首创成立。roadmap 条目 4（Trewartha/Holdridge 并列方案）
  被吸收改指本提案；Köppen 管线保持不动（验证基线不变）。
...
- **`scripts/climate/diagnose_desert_wetness.py`**（§5 诊断裁决轮）：双模式——默认
  ...
  ×3）：季风/κ 消融逐盒归因（B/A、C/A 份额 + Budyko ET/E_pot）。docstring 完整记录
  三族分解、纬向杠杆证伪与 Rodwell-Hoskins/Gill 路径。
