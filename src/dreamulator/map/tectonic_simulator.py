...
def _bfs_distance(
    ...
    *,
    ...
    if adj_offsets is not None and adj_nbrs is not None:
        n = len(adj_offsets) - 1
        while q:
            cid = q.popleft()
            d = dist[cid]
            ...
            if nd >= max_dist_km:
                continue
            for ei in range(adj_offsets[cid], adj_offsets[cid + 1]):
                nid = int(adj_nbrs[ei])
                if 0 <= nid < n and nid not in dist:
                    dist[nid] = nd
                    q.append(nid)
    else:
        while q:
...
def _evolve_cortial2019(
    ...
    *,
    ...
    _adj_offsets = np.zeros(_n + 1, dtype=np.int64)
    for i, c in enumerate(mesh.cells):
        _adj_offsets[i + 1] = _adj_offsets[i] + len(c.neighbors)
    _adj_nbrs = np.zeros(_adj_offsets[-1], dtype=np.int64)
    _pos = 0
    for i, c in enumerate(mesh.cells):
        for nid in c.neighbors:
            if 0 <= nid < _n:
                _adj_nbrs[_pos] = nid
                _pos += 1
        _adj_offsets[i + 1] = _pos
    # Trim if any invalid neighbors were skipped
    _adj_nbrs = _adj_nbrs[: _adj_offsets[-1]]
    _cell_km = np.sqrt(4.0 * np.pi * radius_km**2 / _n)
