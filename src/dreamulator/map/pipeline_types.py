...
class TerrainPipelineConfig:
    ...
    # True = temperature-dependent Γ (moist_lapse_rate): warm ~4.7, cold 6.5
    # °C/km.  Default True (same physics for every world) — the station-implied
    # effective surface lapse splits along temperature, not latitude.  The
    # former subtropical hard band (subtropical_lapse_* + subtropical_dry_
    # warming_c) was removed 2026-09-11: its [15°, 35°] edges printed artificial
    # step lines across Tibet/Andes/Rockies, and both anchor values were wrong
    # (1.5 too shallow in-band, 6.5 too steep in the tropics).
    variable_lapse_rate: bool = True
    ...
    # (Monsoon ΔP column coupling is no longer a config knob: since M4
    # 2026-09-14 the lowland projection factor is derived from cited
    # physical constants — BL-mean amplitude ratio and heat-low depth — in
    ...
    # Calibrated down from the physical 1e6 (which over-transported ocean
    # moisture onto land ~2× the observed ~268 mm/yr).  Shared across worlds.
    moisture_diffusivity_m2s: float = 1e6
