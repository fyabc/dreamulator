...
def extract_etopo1_to_raster(
    ...
    if not tmp_nc.exists():
        ...
        with gzip.open(etopo1_gz_path, "rb") as gz, tmp_nc.open("wb") as out:
    ...
    print(
        f"  Source: {elevation.shape[1]}×{elevation.shape[0]} "
    ...
    if tmp_nc.exists() and tmp_nc != etopo1_gz_path:
        tmp_nc.unlink()
        print("  Removed temporary decompressed file")
    ...
    for y in range(height):
        ...
        for x in range(width):
            ...
            result[y, x] = wr0 * (wc0 * elevation[r0, c0] + wc1 * elevation[r0, c1]) + wr1 * (
                wc0 * elevation[r1, c0] + wc1 * elevation[r1, c1]
            )
...
def build_cvt_mesh_from_grid(
    ...
    print("  Earth actual land fraction: ~29%")
