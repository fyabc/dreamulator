...
EARTH_MASS_SOL: float = 1.0 / 332946.0  # Earth mass in solar masses (M☉/M⊕)
...
def equilibrium_temperature(
    ...
    distributed across the planetary surface before re-radiation.  With
    S = L/(4πd²) the two conventions are:
    - f = 16: full redistribution (uniform temperature, emission from 4πR²).
      Equivalent to T_eq = [S(1-A)/(4σ)]^0.25 (Earth: 255 K).
    - f = 8: no redistribution (dayside only, emission from 2πR²).
      Equivalent to T_eq = [S(1-A)/(2σ)]^0.25.
...
# ===================================================================
# Solar day, seasons, and polar day geometry
# Ref: docs/knowledge/astrophysics/sidereal_solar_day.md
# Ref: Murray & Dermott (1999), Solar System Dynamics (synodic periods)
# ===================================================================


def solar_day_days(rotation_period_days: float, orbital_period_days: float) -> float | None:
    """Solar (synodic) day from sidereal rotation and heliocentric orbital period.

    For prograde rotation (the project-wide convention; the ``Planet`` model
    carries no retrograde flag):

        1/P_solar = |1/P_sidereal − 1/P_year|
    ...
    Args:
        ...
        orbital_period_days: Heliocentric orbital period in days (> 0).

    Returns:
        Mean solar day in Earth days, or None when the body is tidally locked
        to its star (P_sidereal == P_year): the solar day is infinite
        (permanent day/night hemispheres).
    """
    if abs(rotation_period_days - orbital_period_days) <= 1e-9 * orbital_period_days:
        return None
    frequency = abs(1.0 / rotation_period_days - 1.0 / orbital_period_days)
    return 1.0 / frequency


def polar_circle_latitude_deg(axial_tilt_deg: float) -> float:
    """Latitude of the polar circles: ``90° − ε``.

    Poleward of this latitude the body experiences at least one full day and
    one full night per year (for any non-zero obliquity).

    Args:
        axial_tilt_deg: Effective obliquity in degrees [0, 90].

    Returns:
        Polar-circle latitude in degrees (0 = pole, 90 = equator).
    """
    return 90.0 - axial_tilt_deg


def polar_day_fraction_of_year(axial_tilt_deg: float, latitude_deg: float) -> float:
    """Fraction of the year a latitude spends in polar day (midnight sun).

    Zero-eccentricity approximation: solar declination ``δ(t) = ε·sin(2πt/P)``,
    so polar day at latitude φ requires ``δ ≥ 90° − |φ|``.  The fraction of a
    cycle with ``sin θ > c`` is ``(π − 2·arcsin c)/(2π)``.

    Checks: pole (|φ| = 90°) with ε > 0 → 0.5 (half-year day, half-year
    night); polar circle (|φ| = 90° − ε) → 0; Earth's North Cape (71.2°N,
    ε = 23.44°) → ≈ 0.204 of the year ≈ 74 days (geometric; atmospheric
    refraction extends the observed midnight-sun season to ~76 days).

    Args:
        axial_tilt_deg: Effective obliquity in degrees.
        latitude_deg: Latitude in degrees (sign ignored; symmetric hemispheres).

    Returns:
        Fraction of the year in [0, 0.5]; 0 when no polar day occurs.
    """
    if axial_tilt_deg <= 0.0:
        return 0.0
    c = (90.0 - abs(latitude_deg)) / axial_tilt_deg
    if c >= 1.0:
        return 0.0
    return (math.pi - 2.0 * math.asin(c)) / (2.0 * math.pi)
