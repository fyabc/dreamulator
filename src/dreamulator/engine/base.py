...
class BaseEngine(ABC):
    ...
    def output_paths(self) -> list[Path]:
        """Resolved paths of every file this engine writes.

        Used by the pipeline's dirty check (mtime comparison). The default
        implementation lists the declared ``output_files`` under the layer
        derived directory; engines whose outputs span multiple directories
        (e.g. ``maps/``) override this to return the actual files.
        """
        return [self.output_path(f) for f in self.output_files]
    ...
    def run(
        ...
        Args:
            ...
            force: If True, skip all internal caching and recompute.
