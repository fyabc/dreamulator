...
# §5-β 对流临界雨出门结点（骨架 = Neelin-Peters-Hales 2009 物理形态；f_min =
# 9d/150d 深沙漠观测 τ 比）。认识论类别 = 观测拟合；标定域 = **供给修复后的
# W 场体制**（湿槽闭合开启，W 对比度 ≥3）——旧体制（W 全域共位 0.16-0.18）下
# 本门不可标定（09-17 裁决：同 x 档内沙漠需 f~0.1、季风需 f~1 的共位方差在
# R² 上不可调和，「W 场供给对比是硬前置」）。
# **轮 6 重标定（2026-09-24，湿槽轮 5 之后）**：标定输入 = WT5 臂年 W₀
# （湿槽风场、无门——正是联合配置中门的输入场）分箱 vs GPCP，f_tau = 9d/τ_obs
# 盒聚合锚点（`private/research/2026-09-24-pickup-recalib-offline.{py,log}`）：
#   撒哈拉（x 0.18）f_tau 0.08-0.10 → 深压；萨赫勒（x 0.26）f_tau 0.32 → 中压；
#   刚果（x 0.34）f_tau 2.92 → 全开；恒河/几内亚/亚马逊（x ≥ 0.53）f_tau ≥ 1.1
#   → 全开。分箱中位：x < 0.10 的 f_tau > 10 = 湿槽抽干的赤道暖低地共位残余
#   （门治不了，归供给侧）；0.15-0.25 沙漠峰 f_tau ~0.5；x ≥ 0.25 全部
#   f_tau ≥ 1.3 → 开放。旧结点（ramp 0.2→0.8）为旧共位 W 场形，在 WT5 体制下
#   把刚果/亚马逊/几内亚过压 2-3×（轮 5 GW5 刚果 835 的直接成因）。新结点 =
#   陡 ramp 穿沙漠峰（0.12-0.34）、x ≥ 0.34 全开、f_min 锚不变、单调。
#   单曲线纪律不变（陆洋同曲线；陆地临界偏低的 Schiro/Ahmed 语义由沙漠峰
#   位置 0.15-0.25 承载——那正是陆地干柱的 x 居留带）。
_PICKUP_GATE_KNOTS_X: tuple[float, ...] = (0.00, 0.12, 0.20, 0.28, 0.34)
_PICKUP_GATE_KNOTS_F: tuple[float, ...] = (0.06, 0.06, 0.15, 0.50, 1.00)
...
def convective_pickup_gate(
    ...
    陆洋同形，陆地较低的临界由沙漠峰位置（ramp 0.12-0.34）承载；Holloway &
    Neelin 2009（探空印证柱水汽为低对流层湿度 proxy）。Betts-Miller 阈值族为
    参数化对应物。结点标定域 = 供给修复后的 W 体制（详见结点常数注释）。
