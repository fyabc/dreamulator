...
| `fluvial_erodibility` | 2.0 | 1–10 | 参考分辨率下的侵蚀系数 K₀（m/yr）；**造山后存活证据标定**（2026-08-26），见 §6.1 |
...
**有界沉积（稳定性，2026-08-26）**：粗 dt（5 Myr）+ 显式路由下单步载荷巨大，
任何无上限沉积支路都会被放大成数值失控（nacrea 实测：无界兜底把单个海洋
cell 堆到 1361 km）。因此所有沉积支路都带「填充至溢流水位」上限：

- 陆域沉积 ≤ 把该 cell 抬到其下游高程（fill-and-spill，超出部分顺流带走）；
- 水体淤浅 ≤ 到其溢流水位（外洋 = 海平面；内流海 = 周围最低陆缘）；
- 深海扩散（BFS）放不下时依次退回：入口淤至海平面 → 河口 cell（≤ 溢流水位）
  → 仍放不下计入 `discarded`（亚 dt 改道的未解析部分，日志报告）；
- 每步抬升因此有界，多步不再累积放大。代价：被丢弃部分不严格守恒（日志可查）。
...
### 6.1 K₀ 标定（2026-08-26，两轮迭代）

**第一轮：地球瞬时稳态锚点（失败）**。`scripts/calibrate_erodibility.py` 在
ETOPO1 真实地形上标定「复现地球大陆平均剥蚀率 ~0.05 mm/yr」→ K₀=52。
nacrea 实测：**100 Myr 把全球削成准平原**（p50 695→1 m、起伏度 1365→37 m）。
原因：0.05 mm/yr 是地球**构造抬升持续供给维持的稳态值**；顺序管线是
post-orogenic 衰减模式（侵蚀段无抬升补偿），用稳态速率跑衰减必然削平。

**第二轮：造山后存活证据锚点（采用）**。真实证据：乌拉尔/阿巴拉契亚造山后
~300 Myr 仍保 ~1–2 km 起伏；被动陆块剥蚀 0.005–0.02 mm/yr；Cortial 2019 自带
衰减 ε_c = 0.03 mm/yr × z/10km（e-fold ≈ 333 Myr，`tectonic_simulator._erosion`
已实现同款）。K₀ 扫描（nacrea 200k，100 Myr）：

| K₀ | p50 | p95 | max | 判读 |
|----|-----|-----|-----|------|
| 52 | 1.0 m | 37.9 m | 1817 m | 完全准平原化 |
| 20 | 1.0 m | 37.9 m | 1817 m | 同上（准平原化对 K₀ 不敏感） |
| 10 | 1.2 m | 182 m | 3432 m | 低地仍削平 |
| 5 | 3.0 m | 496 m | 5386 m | 低地仍削平 |
| **2** | **56 m** | **1019 m** | **7506 m** | **山系残留 + 低地准平原——造山后世界的物理正确形态** |

**采用 K₀ = 2**（`pipeline_types` 默认，nacrea 与地球同一物理）。低地准平原
是造山后 100 Myr 无抬升补偿的物理正确结果；要维持起伏需抬升补偿——
构造-侵蚀 Δt 耦合（抬升率场）已立项（roadmap §6 P1；调研见竞品分析 §4.2.2）。
`scripts/calibrate_erodibility.py` 保留为地球瞬时速率测量工具（其稳态目标
不适用于顺序管线，文档已注明）。

### 6.2 nacrea 基线（2026-08-26 定稿，K₀=2、m=0.5/n=1.0、**10 Myr**、20 步）

`surface_evolution_time_myr` = **10**（nacrea 定位：构造活跃、地表更新快，
「最近一次构造幕以来的侵蚀时长」；轻微侵蚀 + 重点是河网计算——用户反馈
100 Myr 版本「遍布深谷不自然」后定时长，不降 K₀，保持全球同物理）。

| 指标 | 数值 | 判读 |
|------|------|------|
| 高程分位（侵蚀前→后） | p50 901→805 m；p95 2162→2114 m；p99 3428→3306 m | 山系基本未动、低地轻微削平 |
| 起伏度 p95−p50 | 1261 → 1309 m | 河谷下切、山脊保留 → 起伏略增 |
| 深尾 | min −1858 m、p1 −953 m | 大陆级汇水干流的深谷（可见但收敛） |
| 陆域平均净变 | −79 m（0.0079 mm/yr） | 被动陆块剥蚀档（0.005–0.02）内 |
| 沉积 | 陆域 61,415 km³（3518 cells）；水体 1.53e7 km³ | 质量守恒（§5.5）；陆地占比 28.8%→32.7% |
| 气候回归（vs 侵蚀前地形） | T −0.14 °C（陆 −0.11）、P −68 mm；Af +1619、BSh +2010 | 漂移小，量级可接受 |

**已知效应记录**：① 100 Myr 无抬升补偿会把地形削成准平原（实测，见 §6.1
扫描）——维持起伏需抬升率场（roadmap §6 P1 已立项）；② 沉积物淤在洋底
——多构造旋回尺度需要沉积物汇（俯冲带走），远期项；③ 河网计算（`river_id`/
`river_order`/features.json）不依赖侵蚀强度，侵蚀降为轻微后河网照常产出。
