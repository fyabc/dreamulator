...
- **dreamulator 适配性**：★★★☆☆ — 当前引擎是单稳态（给定参数 → 唯一气候），没有建模「历史依赖/迟滞」。这是比「分类」更深的一层：引擎是否该输出「可能落入的多个气候态」。与 nacrea 慢自转（接近 1:1 锁相）直接相关——按 Forget & Leconte，nacrea 可能已处于「无双稳态」的临界自转区。
...
> **2026-09 现状**：项目的气候描述主线已转向统一气候描述（UCC）——连续描述量 +
> 版本化分类 profile（见 `ucc_climate_descriptors.md`）。UCC 不再追求与任何单一
> 分类体系的一致率；Köppen 保留为并列参照列。本节 §3.1–§3.3 的「多体系透镜」
> 设想是 2026-08 的调研结论，其中 Trewartha / Holdridge 的实施条目尚未排期，
> 是否仍需要以独立透镜形式实现，待 UCC 着色与用途验证完成后重新评估。
