...
（roadmap 3A.3a 中期项；现状与后续见 `climate-pipeline.md` §13 已知局限）。
...
受科氏力偏转成**西风**——索马里急流 / 几内亚西风带。`cross_equatorial_monsoon_wind()`
在跨赤道带内**只替换背景纬向风**为该西风（保留背景经向辐合结构）：

```
u_scale = ε_u · Ω a · sin(φ_itcz_max)      # 西风（科氏偏转）
```

- `Ω a` = 赤道自转线速度（角动量标度），`sin(φ_itcz_max)` = ITCZ 最大迁移的正弦
  （太阳加热强度），`ε_u ≈ 0.015` = 边界层摩擦衰减（物理常数，共享所有世界——
  慢自转/小倾角世界自动得弱西风）；
- 幅度只依赖 ITCZ 最大迁移纬度（非局地纬度），钟形 `g = sin(π·|φ|/|φ_itcz|)`
  （赤道 0 → 峰值 → ITCZ 0）。这是对子项 1 `u = f·v_n/k_d` 的**量级替换**——后者随局地
  f 增长，一旦季风槽北移就会过强 ~6×。

符号在两半球恒为西风，赤道与跨赤道带之外为零——不动信风与中纬 Ferrel（技术债 24「扫掠
雨带」否证来自移动整个环流，这里是局部反转）。

**标定锚点**：几内亚湾 / 印度 7 月地表西风 ~+2 m/s（NCEP sig995；δMCD/δTH 分解
`diagnose_moisture_decomp.py` 结论 ② 的直接修复）。跨赤道西风随 ITCZ 逐月迁移，作为
`wind_monthly` 的季节支——年平 `wind` 是逐月风矢量平均导出的「平均西风」（「逐月为主、
年平为导出量」契约，B0b）。

**已否证方向（防重试，2026-09-14）**：跨赤道流的**经向支**（v 指向 ITCZ）与**季风槽
北移**（陆地温度峰值判据）——v 向北缺「ITCZ 北侧向南」的辐合结构导致几内亚辐散，
季风槽用温度峰值无法区分湿润对流 vs 干热沙漠（撒哈拉误判北移）；两者属「稳态耦合」
大工程（`climate-steady-coupling.md`），函数 `monsoon_trough_latitude()` 保留供未来参考。
恒河/华南的西南/东南风供给缺口因此仍开放（归 LLJ 子项 3 + ④ 反气旋）。
...
- 跨赤道西风已补方向（§4.6），但低空急流（LLJ）垂直结构仍未解析——水汽通量的
  850 hPa/地面 ~1.8× 量级缺口是单层风的已知残留（δMCD/δTH 结论 ③，D/F 子项 3）；
...
| 跨赤道季风西风带 | `engine/monsoon_circulation.py:cross_equatorial_monsoon_wind()` | ✅（ε·Ωa·sin(φ_itcz_max)，随 ITCZ 逐月） |
