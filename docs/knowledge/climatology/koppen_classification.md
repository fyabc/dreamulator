...
### 4.1 前端显示名约定（i18n，2026-09-21 查证）

中文（zh-CN/map.json）按**大陆地理教科书惯例**而非 Beck 英文直译：B 群第三字母
h/k = 热/**温**（k = kalt 寒冷温度型，年均温 < 18 °C、最冷月 < 0 °C，实质是温带
干旱——教科书通名「温带沙漠气候 / 温带草原气候」，故 BWk = 温带沙漠、BSk =
温带草原(半干旱)；英文侧保留 Beck 原词 Cold desert / Cold semi-arid）。季节后缀
系统化：a = 热夏、b = 温夏、c = 冷夏、d = 严冬——Dfc = 亚寒带针叶林(冷夏)、
Dfd = 亚寒带针叶林(严冬)（旧译「亚寒带 / 极端大陆性」两名不成体系，已修）。
依据：中文维基「温带干旱半干旱气候」、百度百科「亚寒带针叶林气候/副极地气候」、
Beck et al. (2018) 图例（英文）。
...
| 三字母 b/c 月数判据（≥4 / 1-3 月 ≥10°C） | `koppen_classify(t_months_ge10=…)`，由单一权威的 `t_monthly`（B0c 交接 + 终聚合）计数 | ✅ |
...
| 月度极值来源 | `climate_seasonality.py:compute_seasonal_climate()` + B0c 交接（水平值取年平场）；年温 = ⟨t_monthly⟩ 终聚合（⟨t_monthly⟩ ≡ t_mean 由构造成立） | ✅ |
