...
### 6.1 大陆与边界真实感批次（2026-08，用户反馈驱动）

- **克拉通低地化**：板块内部陆地仅取 0.4× 均匀板块偏差，其余换为多尺度
  "动态地形"起伏（`continental_undulation_m`，默认 600 m，地幔对流类比
  ±500 m，Flament et al. 2013）。gaia-m：>2000 m 陆地占比 29.7%→12.3%
  （地球 10–15%），均陆高 1615→1238 m（地球 ~840；残余偏高对应快板块年轻世界）。
- **洋中脊归位**：离散洋脊隆起 0.35×（脊顶 ≈ −2500 m，地球同）并对板块偏差
  部分解耦（−0.6×off）；泄漏的大陆地壳孤立 cell 在 base 阶段前重标为洋壳
  （`_relabel_leaked_crust`），洒点岛屿改为仅由岛弧/热点/钉扎涌现。
...
- **gaia-m 配置**：南大洋四环洋带（压制设计外自发出陆南方大陆）、南极浅海
  −120 m / 地峡 +120 m 钉扎、`boundary_warp` 0.3、`boundary_uplift_noise` 0.8。
