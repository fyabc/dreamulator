...
## 3 极地偏暖偏湿（P1，quick win）

**症状**：88N 温度 −1.8 vs 观测 −15.6（+13.8）；78N +7.5；−68~−78S +7~+10.5；
−88S 降水 756 vs 145 mm（+611）、88N 350 vs 184；EF→ET 848 + Dfc→ET 1408 混淆。

**根因假设（待验证）**：
1. SST 高纬剖面偏暖——`_ocean_surface_temperature` 的海冰 sigmoid（中心 70°、宽 8°）
   过渡太早/太平，或海冰范围不足；
2. 寒带降水缺**低温物理钳制**——−20°C 空气持水量撑不起 350+ mm/yr
   （Clausius-Clapeyron，`docs/knowledge/climatology/precipitation.md` 相态节）；
   疑为 κ 扩散把过量水汽送进极区后雨出率没跟温度走。

**参照**：ERA5 极地降水 ~150–200 mm/yr；ExoPlaSim 极地处理（private/external-projects/exoplasim_poc）。

**方案方向**：海冰剖面校准（对齐 ERA5 海冰边缘）+ 雨出率低温抑制（可推导：饱和比湿
随温度指数衰减，雨出效率 ∝ 可降水量）。独立性最好，先做。
