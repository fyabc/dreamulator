...
### 2.7 深时演化门控（氧阈值、false dawn 与多次多细胞化）

§2.3 的谱系拓扑描述「谁是谁的后代」，本节补上它的**事件维度**：演化树上哪些转移在什么
环境条件下才允许发生。地球的前埃迪卡拉记录（详见
`../../knowledge/ecology/pre_ediacaran_macrofossils.md`）给出的核心图景是：宏体生命不是
在某个时刻一次性解锁、然后一路向前，而是与一段段瞬态富氧窗口耦合的多次独立实验——
窗口打开时兴起，窗口关闭时消失且不留后代。如果生态层要能复现这种时间线（而不是
「单调进步」的默认叙事），骨架层应预留以下机制。这些机制归 P2/P3 的演化模块，当前不实现：

| 机制 | 设计意图 | 地球参照 |
|------|---------|---------|
| **氧阈值门控** | 宏体生物的出现由氧浓度按阈值（而非线性）门控：体型天花板随氧非线性升高，低于约 1% 现代大气水平的量级时只允许微生物与微型真核生态。这天然产生长停滞期 | 大氧化事件后宏体仍长期缺席；boring billion |
| **瞬态富氧窗口 → false dawn** | 碳埋藏或大火成岩省风化脉冲把氧抬进窗口 → 窗口内解锁一次宏体辐射；脉冲结束 → 该谱系灭绝或缩回微型。演化树必须允许「没有任何现存后代的早期实验支」存在 | 弗朗西维利安生物群 2.1 Ga 兴起后消失，无后代 |
| **多细胞化多次独立起源** | 多细胞化建模为可重复发生的状态转移，每次起源可用不同的发育机制与体型构建规则；明确禁止「单一多细胞祖先推出全部宏体后代」的树假设 | 藻类、真菌、动物的多细胞化各自独立 |
| **双阈值阶梯** | 区分「能力获取」与「生态兑现」两个阈值：有性生殖、细胞分化等工具箱可在小体型下提前数亿年获得，大体型要等更高阈值才兑现；两阈值之间即 boring-billion 式停滞 | 1.05 Ga 已有有性生殖的红藻，远早于宏体动物 |
| **保存窗口 ≠ 存在** | 可观测化石记录与真实多样性分成两个字段：前者 = 后者 × 保存概率，保存概率由独立的埋藏条件（缺氧黑色页岩、磷酸盐化等）决定。这会自动产生「化石记录突然跳跃」的假象，供世界内学者争论 | 特殊埋藏窗口的分布极不均匀 |
| **化学化石置信度** | 分子指标（甾烷等）赋「同源置信度」参数：它可能来自干群代谢物（早于冠群数亿年），也可能被污染伪造。允许世界观内的学者先被错误分子证据误导、后被推翻 | 1.6 Ga proto-ursteroids；2.7 Ga 甾烷被证伪为污染 |
| **环境闸门 + 生态放大器串联** | 辐射分两阶段：先由氧/营养盐解锁可能性（底向上），再由捕食、牧食等军备竞赛把可能性放大成辐射（顶向上）。串联后可复现「瓮安→埃迪卡拉→寒武纪」的三段式节奏 | 同上 |
| **LIP 双重角色** | 大火成岩省作为可复用的事件模板：风化输入营养、抬升生产力与氧时是辐射的引信；CO₂ 升温叠加海洋脱氧时是窗口的关闭者 | Chi Fru et al. 对弗朗西维利安盆地的营养盐模型 |
...
| P3c 图像输出 | body plan → 3D 生物样例与视频素材（管线见下节） | P3a，独立模块（类比 `narrator.py`） |

**归属**：P3a/b 留主仓（见 §0.2 决策 #5）；P3c 单独成模块。是否抽 `packages/specbio`
待生态层 P1/P2 骨架成熟后重评。

#### P3c 呈现层设计输入：3D 生物样例与视频管线（2026-09-17）

这一节回答「异星生物推演出来之后怎么呈现」，双重目的：给生态引擎做一套 3D 渲染器
（生物样例是引擎产物的可视化），同时为将来的 B 站科普视频准备高质量素材——两者
同源，共用同一批资产。

**核心架构原则：3D 生物是引擎的渲染器，不是手工一次性作品。** 生态层从世界物理
（重力、大气、温度季节性、光学环境）推导生物参数，输出是一个 creature spec 数据
模型（体型、肢体方案、代谢策略、体表覆盖等）；呈现层的职责是把它变成 Blender 场景：

```
ecology 引擎 → creature spec (JSON) → Blender Python 脚本 → 程序化建模/材质/绑定
```

Blender 有完整的 Python API，与本仓技术栈天然同构。这样做的好处是生物形态的每个
视觉特征都能追溯到推导它的物理输入——这正是 Biblaridion 在《外星生物圈》系列里靠
手工维持的严谨性（他的 3D 示意图证明了一件事：同一个设定，文字描述和一张光线
讲究的渲染图，说服力差一个量级），我们用管线把它自动化。

**AI 的辅助位置（三层，不碰推导）**：概念设定图用 Midjourney/SD 系快速探索造型
方向；基底网格用文生 3D 服务直出再修（2026 年的工具格局：Tripo/Meshy 出得快，
Rodin/Hunyuan3D 几何质量更高，Hunyuan3D 开源可自部署；业界共识管线就是「AI 出
基底 → Blender 修拓扑、UV、PBR 纹理」）；纹理贴图同样 AI 生成后进 Blender。推导
层（物理约束 → body plan）始终是引擎的纯函数，AI 不介入。

**渲染与本机硬件适配**：当前开发机是 AMD 集成显卡，Blender 视口重雕刻和 Cycles
的 GPU 渲染都会很吃力。对策按优先级：EEVEE 优先（1080p 目标下足以出电影感，渲染
比 Cycles 快一个量级）；静帧需要物理光栅时用 Cycles 走 CPU 夜间渲染；动画镜头上
云渲染农场（按帧计费，便宜）；文生 3D 基底本来就在云端跑，不受本机限制。

**视频管线（B 站 1080p、目标「大片感」）**：Blender 负责动画与渲染，DaVinci
Resolve（免费版够用）负责剪辑与调色——调色是「大片感」最大的单一杠杆。画面上
的「大片感四件套」是 HDR 环境光、景深、体积光、运动镜头（摇臂/对焦变换），都
在 Blender 内解决。AI 视频生成（可灵/海螺/Runway 这一档）只用于氛围空镜和
b-roll——主体生物镜头不依赖它，因为可控性差、科学准确性无法保证，与本项目
「严谨推演」的定位冲突。音频侧自己录或 TTS 配音，配免版权音乐音效；字幕用
剪映自动字幕或 Resolve。

**资产管理**：creature spec 的 JSON 与「spec → Blender 场景」的导出脚本入库存
版本化（与引擎其他产物同源）；.blend 工程文件和渲染出的图/视频是构建产物性质，
按 `maps/`、`derived/` 的同款策略 ignore，发布走资产包而不是 git（.blend 体积
大，参考 LFS 教训不进常规提交）。
