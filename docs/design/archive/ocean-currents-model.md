...
1. **模型选型：Stommel 流函数解（β 平面摩擦涡度方程）为主干，Ekman 层做诊断**。
   ...
   nacrea 慢自转（Ω=0.31Ω⊕、3.25 天潮汐锁定）下地转求逆会除零，流函数是唯一
   全程良态的极小模型；② 西边界强化（WBC）作为摩擦边界层自然涌现，不需要
   手贴 ×3 系数（只留校验阈值）；③ 海峡闸口可在流函数框架内以"跨盆地输运
   约束"表达，直接对接 nacrea 临界洋流剧情。
...
5. **验收锚点**：温度 RMSE 12.87°C → <8°C（roadmap M3，earth/climate-dev 离线验证）；
   nacrea 四条临界洋流用例（heightmap 计划 §4.4）。
...
| `docs/knowledge/climatology/ocean_currents.md` | 科学底座齐备：§1.1 Ekman、§1.2 Sverdrup/Stommel、§1.3 慢自转环流更大（nacrea 适用）、§2 热盐双稳态、§3 海峡闸门（ITF/巴拿马/德雷克）、§5 引擎对应表（现状 ❌ 待填） |
...
- **临界性**：nacrea 版直布罗陀 analog，但机制不同。不受冰期海平面
...
- 南大洋环极通道（~50°S）：宽阔深水，无狭窄瓶颈。nacrea 两圈环流下
...
把 nacrea 洋流压到 ~0.9 cm/s，实际应为 ~46 cm/s）；这就是可视化与热输送用的洋流场
（正压近似，深度平均）。
...
若慢自转 nacrea 下 CG 不收敛或收敛到非物理解，有一个降级逃生路径：
...
但最终须切回 Stommel——因其"海峡输运定量表达式"是 nacrea 临界洋流需求的
...
| nacrea 形态目检 | 环流圈数目/尺度 | 慢自转预期：更少更大（不照搬地球五圈） |
| nacrea §4.4 用例 1 | 海峡 150 m 深、海平面 0 → 流量 >0，两侧 SST 梯度被抹平 | 通过 |
| nacrea §4.4 用例 2 | 海平面 −120 m → 通道关闭、两侧 SST 分异、全球均温偏移 | 通过 |
...
| **P1** | 引擎集成：simulate_climate stage 2.5、VoronoiCell 字段、config 段（删 num_gyres）、回写清单、climate_summary 追加段 + 5.2 集成测试；`ekman_current_direction` 向量化并接线（climate-pipeline.md ⏳ → ✅） | 2–3 天 | build nacrea 全绿、关闭开关逐位回归 |
| **P2** | 标定与验证：earth/climate-dev RMSE 调参（R、passes、λ）、nacrea 形态目检 + 用例 1/2、`ocean_current_speed.png` 导出 | 2–3 天 | RMSE <8°C；nacrea 用例通过 |
| **P3** | 海峡闸口精修（若 P0 的简化版不足以过用例 2）：盆地识别、海峡扫描、输运约束耦合；nacrea 前导点褶皱山系钉 3–5 个浅峡 feature（geography.yaml 迭代） | 3–5 天 | §4.4 用例 1/2/3 |
...
| 风险 | 对策 |
|------|------|
| 慢自转下 β 弱 → ψ 幅值过大/病态 | β 用 2Ωcosφ/a 全幅值；对 ψ 做物理上限幅（输运上限 ~100 Sv 量级截断并 warning）；标定阶段观察 nacrea 实际环流尺度 |
...
| 图 Laplacian 均匀权重的边距比近似（l/d ≈ 常数）在剧烈非均匀 cell（如 pentagon）处失效 | CVT after Lloyd 的 cell 面积 CV <5%——近似有效；若 nacrea 局部出现畸形 cell（pentagon 聚集在边界），**Laplacian 改用 1/(A_i×d_ij) 加权**（改动量：一个权重数组替换常数，稀疏装配同一 loop） |
...
| **CG 不收敛 / 收敛到错解**（极端参数组合或慢自转+复杂盆地形状） | 有 §2.2.2 的启发式环流作为逃生出口（纯几何、零求解、半天原型验证）；若触发降级，则牺牲海峡输运定量精度（nacrea §4.4 用例退回定性验收） |
...
| 海峡命名配置（碎门暖流等作者命名） | 字段已留，配置化随 nacrea 剧情迭代 | P2 前导点褶皱山系精修时顺手 |
