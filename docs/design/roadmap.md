...
5. **地形：均匀高原** — `terrain_synthesizer` 每板块叠加
   ...
   （低地为主 + 盾地/造山带）。
6. **板块大小分布过于均匀** — 初始 Voronoi 剖分（Poisson-disc 均匀种子）产生
   近乎等大的板块（实测 20 板 CV≈0.33、max/min≈3.9）；裂解只把板块切成相近大小，
   无法产生真实地球的幂律/偏态分布（太平洋板块 ≈ 最小板块的 ~100×）。
   方向：非均匀种子分布（按面积偏置采样）、裂解时按偏态分配子板块面积。
7. **板块边界过于平直** — 板块边界是 Voronoi 边（种子间测地线/大圆弧）；
   boundary_warp 噪声扭曲只加小尺度锯齿，缺乏大尺度弯曲。真实俯冲带因 slab pull
   弯曲成弧（阿留申弧、日本弧）。方向：俯冲边界加弧状曲率、或非 Voronoi 剖分。
   （注：板块数过少 bug 已修——重分区命名错配 + needs_resample 每步触发，
   修后 6→35 板；但大小/边界形态问题见本条与上一条。）
8. **缺少俯冲海沟，海洋最大深度偏低** — 汇聚边界只做抬升（造山），未在大洋
   俯冲处生成海沟凹陷；gaia-m 海洋最深仅 −4758 m（地球海沟 ~−11000 m），
   无 <−6000 m cell。方向：洋-洋/洋-陆汇聚边界沿线生成线性海沟凹陷。
