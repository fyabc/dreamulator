...
| 洋流 + 温度精细化（3A.3） | 🚧 | v0.19.0：Stommel 正压流函数 + GMRES 求解（全球风生环流已上线）；冰盖反照率 / 云反馈 / 海峡参数化待完成 |
| 慢自转经向输送（3A.3a） | 🚧 | v0.19.0：Ω^(-1/3) 风速标度 (Hill 2019) + 经向风分量 ✅；Hadley 宽度参数化已有 (`hadley_extent_deg`)；ΔT(Ω) 参数化 + 扩散经向热输送待完成 |
...
| 潮汐锁定经度效应（3A.7） | 🚧 | v0.19.0：`sub_planet_warming_c` 参数化 cos 暖化 ✅；次恒星点固定经度温差待建模 |

### Phase 3A 子任务分解（2026-08 自 climate-engine.md 旧 §6 收敛）

**3A.3 洋流 + 温度精细化**（物理链：风场 → Ekman 输运 → 大洋环流 → 热输送 →
沿岸温度修正；目标温度 RMSE <8°C）：

| 子任务 | 说明 | 验证目标 | 状态 |
|--------|------|---------|:----:|
| 风生表面流 | Stommel 流函数 + GMRES 求解（替换简化 Ekman 方案） | 五大环流圈形态 | ✅ v0.19.0 |
| 西边界强化 | WBC 摩擦边界层自然涌现（Stommel 方程 β/∇² 项） | 湾流/黑潮/巴西/阿古拉斯识别 | ✅ v0.19.0 |
| 沿岸热输送 | 沿流 SST 平流松弛（暖异常向极、冷异常向赤） | 西欧冬季偏暖 | 🚧 SST advection 已实现，幅度弱 |
| 上升流 | 东边界离岸风 → Ekman 输运诊断量 | 纳米布 / 阿塔卡马更干冷 | 🚧 诊断量已实现，未接入 EBM |
| 季风洋流反转 | 流向季节反转（索马里洋流型） | 季风区洋流季节反转 | 📋 依赖季节循环 |
| 冰盖 / 云反照率 | 年均 T<-10°C → 反照率 0.7；热带对流区云 +0.1 | 极地更冷 / 赤道不过热 | 📋 |
| 海拔-温度解耦 | 高海拔低纬独立温度廓线 | 青藏高原正确 ET/H | 📋 |
| Hadley 经向风 | 地表信风 equatorward 分量（Hadley/Ferrel/Polar） | ∂τ_n/∂x_e 旋度源 | ✅ v0.19.0 |
| Ω^(-1/3) 风速标度 | Hill et al. (2019) 慢自转风速增强 | gaia-m 1.48× 风应力 | ✅ v0.19.0 |
| 次行星半球暖化 | 潮汐锁定卫星的 sub-planet 经度温差 | gaia-m Aegis 1°C 暖化 | ✅ v0.19.0 |

**3A.3d 降水物理（BFS 水汽）**：翻山衰减（elev_gain>500m 按比例阻挡至 0.8；
连续山脉逐跳 ×0.5 指数累积）→ 验证安第斯西坡 <100mm、巴塔哥尼亚雨影；
潜在植被蒸散循环（EMIC 一步参数化：recycling = 0.5·σ((T−5)/3)·σ((P−400)/200)，
雨林 ~50%、撒哈拉 0%）接入 BFS 蒸发后**删除人工热带底线**——完成标志：
删底线后 Aw 准确率 >30% 且无 800mm 人工尖峰。

**3A.4 空间格局精细化**（目标空间准确率 >55%、Kappa >0.4）：西岸/东岸不对称
（中纬西岸 +500mm）/ 雨影精确化（Föhn −70%+）/ 内陆干旱梯度（>1500km 指数衰减）/
沿海雾降水（纳米布 P<50）。

**3A.5 海洋气候分区（Longhurst）**：Phase 1（不依赖洋流）POLAR / WESTERLIES /
TRADES / COASTAL 四 biome；Phase 2（依赖 3A.3）WBC / EBC / SHELF / GYRE_CENTER /
EQUATORIAL / POLAR_ICE 子省；输出 `VoronoiCell.ocean_province` + 前端 categorical 图层。

**验收里程碑**：M1 热带修正（A 类 >50%、空间 >30%）→ M2 季节变化（D 类出现、
C >30%、Kappa >0.3）→ M3 温度精细（RMSE <8°C、B→C 混淆消除）→ M4 空间精细
（>55%、Kappa >0.4）→ 合并 main。

**长期愿景**：简化 GCM（2D 浅水 / 谱方法）、动态植被耦合、古气候（米兰科维奇
扫描）、极端系外行星（潮汐锁定 / 高 CO₂）。
...
5. **海岸线过于平直** — 海陆判定在 cell 粒度（~76 km @ 100k cells），海岸线
   ...
   格局，此改进只增海岸微观粗糙度。
5. **地形：均匀高原** — ~~每板块均匀偏移成整板高原~~ 已大幅缓解（2026-08）：
   板块内部陆地仅 0.4× 均匀偏差 + `continental_undulation_m` 多尺度起伏后，
   gaia-m >2000 m 陆地 29.7%→12.3%、均陆高 1615→1238 m（地球 10–15% / ~840 m）。
   残余：均陆高仍偏高（快板块年轻世界的合理残留）；流水侵蚀落地后可进一步
   雕刻板内纹理（见 tech-debt/realism 批次与 terrain-pipeline §6.1）。
...
9. **大裂谷海过于对齐经线、边界平直** — 当前用单个拉长偏置场（elongation=11、
   ...
   **~~另一脆弱机制~~** → ✅ 已修复（2026-08-07，feat/geography-elevation-anchor）：
   ...
   （冰期/临界海峡实验），见 terrain-pipeline.md §3.5。
   **蜿蜒裂谷原语**仍 open（上段）。
...
- `docs/design/archive/map_system_design.md` — 早期 ADR（已归档）
