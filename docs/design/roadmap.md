...
7. **侵蚀应能冲开浅海海峡封堵**（2026-08-25 转为能力需求）— 北方内海海峡当前为
   畅通状态（geography.yaml 微调）；但后续地质输入微调可能再次使浅海海峡被封堵
   （低陆 cell 构成"堤坝"，隔断内海与外洋）。能力需求：侵蚀（3B）实施后，此类
   封堵应能被自然下切冲开，不依赖人工在 geography.yaml 加 `shallow_sea` feature
   强行切开。验收：人工构造封堵场景 → 跑侵蚀 → 海峡被冲开。
