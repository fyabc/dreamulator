...
25. **经纬向东西约定不一致（约定审计）**（2026-08-29 季风攻关中发现）— 网格经度取
    `lon = atan2(z, x)`，科氏参数取 f = +2Ω·sinφ（旋转轴 +y）。在该组合下地面自转
    朝经度减小的方向，即这颗行星的物理东是经度减小的方向，经度标注本身是镜像的。
    数值验证过的现状：
    - **大气核心物理正确**：`hadley_cell_wind`、`_geostrophic_wind`、水汽预算使用的
      矢量约定（east = north_t × r̂，north_t = (0,1,0) 的切向投影）构成右手 ENU 基，
      信风向西、中纬西风、北半球科氏右偏、低压在左，全部符合气象学。
    - **`map/ocean_circulation.east_north_basis` 指向物理西**：实现返回经度增大方向
      并称其为 east（其 docstring 写的公式 k̂×r̂ 才是物理东，实现与文档自相矛盾）。
    - **靠双重翻转恰好自洽的下游**：Step 6.6 海岸不对称（`_detect_coastal_cells` 的
      经度标签翻转 × uzonal 基底翻转，结果正确）；风场写回单元格的 `_we = -_we`
      硬翻转（FIXME）；Stommel 环流（β 项与风应力旋度各翻一次，方程基本不变，但
      摩擦项不翻，西边界强化的位置可能有误，需重建验证）。
    - **目前确认为错的下游**：Föhn 步骤按经度标签从上风侧的反方向找地形；上升流
      指数把「经度标签东岸」当成物理东岸，选错了海岸；洋流显示分量符号与真实东相反。
    - 修复方向：`east_north_basis` 按 docstring 改为 k̂×r̂（一行），删除风场写回处的
      FIXME 硬翻转，Föhn/上升流改用矢量或物理东判定，Stommel 与洋流显示做前后对比
      重建验证。季风模块（`engine/monsoon_circulation.py`）已按物理东约定实现，不受
      此影响。排期：季风定稿后单独攻关（避免与季风的验证重建混在一起无法归因）。
