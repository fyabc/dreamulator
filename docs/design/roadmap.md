...
| 气候精度 | Köppen 空间准确率 30.8% / 30 类 Kappa 0.265 / 群组准确率 65.8%（主指标，逐 cell vs Beck 2018，200k ETOPO1）；分布匹配 67.5%（辅助）；T2 端元复现 4 测试 + T3 物理合理性 7 测试已建 |
...
| **P0** | **风场镜像约定修复**（技术债 24 升级，2026-09-12 实锤）：水分收支/地形雨消费镜像风 → 水分平流东西反转 = 地球逐格 P 三大偏差主因 + nacrea 东岸沙漠；消费点翻转 + 符号锚定测试，见 [climate-layer-improvement.md](proposals/climate-layer-improvement.md) §3 | 2–4 天 | ★★★★★ |
| **P0** | **季风动力学与季节风场**（技术债 23，§七）：v1 已落地；环流胞圈逐月迁移与月度矢量场（技术债 24）合并推进，方案底稿 [climate-layer-improvement.md](proposals/climate-layer-improvement.md) §2，验收 Cwa/Cfa/Am recall >40%；**镜像修复后孟加拉归因需重估** | 剩余 1–2 周（随 24） | ★★★★★ |
...
24. **月度矢量场：风场先行、洋流不伪造**（2026-08-28，与 23 同源）— 现状：
    ...
    - **月度 SST（居中）**：洋流季节化落地前，月度海表温度/混合层温度、上升流强度、
      ...
      渔场）与文明层（季风航线）提供输入。
    - **风场符号约定 FIXME（`climate_simulator.py` 存储处 `_we = -_we`）**：内部
      `hadley_cell_wind` 在镜像东基（`east = north × r̂`，指向物理西）上合成风矢量，
      存储 `wind_east_m_s` 时翻转东分量补正；整条 Stommel 洋流链按内部镜像约定校准
      （喂真东基风场会使洋流全场镜像——2026-09-08 earth 导入器实测确认）。该 FIXME
    ...
    - **镜像 bug 实锤，升级为独立 P0（2026-09-12 审计）**：`_compute_precipitation_monthly_budget`
      内水分收支（年平+逐月）、地形雨 `_wind_unit_m`、Step 6.6 海岸不对称 `_uzonal`
      三处消费**未翻转的镜像风** → 水分平流/迎风岸判定东西向反转（Föhn 6.7 已翻转不受影响）。证据：地球逐格 P 三大偏差全是镜像签名（索马里 +852 镜像向岸/孟加拉
      −1975/阿拉斯加 −587；巴塔哥尼亚迎风:背风应 ~12:1 实测 1.05:1）、nacrea 东岸
      信风迎风反而沙漠（20-40N 西岸 C 55% vs 东岸 B 47%，#48744 暖沿海 BWh）。
      **镜像对纬向指标/整体 R² 不可见**（08-29「季风镜像误报」裁决的根因）。修复 =
      仅在该两处消费点翻转东分量（4.1-B `_wind_phys` 同款配方），**不动源头/Stommel/
      存储链**（历史踩坑 = 源头翻转导致前端方向全反）；符号锚定测试先行。根部统一
      仍归本条月度矢量场轮。方案与验收见 proposal §3。
