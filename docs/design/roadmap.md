...
| **P1** | **构造-侵蚀 Δt 耦合（抬升率场）**（2026-08-26 立项）：把「构造段→侵蚀段」顺序管线改为按 Δt（2–5 Myr）交替的耦合循环——每步：抬升率场应用（会聚速率→抬升率物理标度，会聚带 ~0.1–1 mm/yr、裂谷沉降、转换 ~0）+ 侵蚀/沉积一步（隐式格式无条件稳定，1 步/Δt）+ 海平面每 N 步重标定。业界主流做法（Landlab 循环结构；Badlands 由 GPlates 单向驱动；调研见 `competitor-analysis.md` §4.2.2），≈ 把 Cortial 预留的水力侵蚀接口补上。解决顺序管线的准平原化问题（无抬升补偿时地球锚定侵蚀削平地形，2026-08-26 实测） | 1–2 周（运行 ~10–20 min @200k） | ★★★★ |
...
7. **侵蚀应能冲开浅海海峡封堵**（2026-08-25 能力已实现 ✅）— 北方内海海峡当前
   畅通；若后续地质输入微调再次封堵浅海海峡（低陆 cell 构成"堤坝"），侵蚀会把
   封堵坝下切至海平面，结束时的海侵缺口通道冲开海峡：minimax 鞍部路径识别 +
   海平面基准面下切（封堵坝豁免河道宽度稀释，集中缺口下切）+ 鞍部降至
   `sea_level − 5 m`（见 `docs/knowledge/geology/erosion.md` §3.1/§3.2）。
   验收测试：`tests/test_map/test_erosion.py::test_erosion_breaches_dammed_basin`。
