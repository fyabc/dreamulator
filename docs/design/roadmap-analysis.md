...
| 问题 | 改进方向 |
|------|---------|
| ~~板块洪水填充产生碎片化大陆~~ | ✅ 已修复：Cortial 2019 Voronoi + 时间演化 |
| ~~大陆位置随机，无纬度偏好~~ | ✅ 已修复：中低纬度大陆面积更大，极区以海洋为主 |
| ~~大陆面积分布不合理~~ | ✅ Voronoi 自然分布 + seed 漂移 |
| ~~无超大陆/裂解周期概念~~ | ✅ 已修复：可选的"超大陆初始态"模板（如 Pangea-like），再自动裂解 |

#### 2.5b：山脉与地形特征改进 ✅

| 问题 | 改进方向 |
|------|---------|
| ~~山脉仅出现在板块边界~~ | ✅ 已修复：热点火山链（如夏威夷-皇帝海山链）、地壳穹隆 |
| ~~边界山脉的高斯衰减过于平滑~~ | ✅ 已修复：不对称剖面（迎风坡陡、背风坡缓）+ 更高的峰谷比 |
| ~~无高原/盆地/裂谷等特殊地貌~~ | ✅ 已修复：在汇聚/离散/转换边界分别生成对应的典型地貌组合 |
| ~~无冰川侵蚀痕迹~~ | ✅ 已修复：高纬度/高海拔冰川地形（U 形谷、冰斗、角峰） |

#### 2.5c：海岸线与大陆架 ✅

| 问题 | 改进方向 |
|------|---------|
| ~~海陆过渡生硬（无大陆架）~~ | ✅ 已修复：指数衰减的大陆架深度剖面 |
| ~~无峡湾/溺谷/三角洲~~ | ✅ 已修复：基于河流出口 + 潮差参数的微型海岸特征 |
| ~~岛屿弧/海沟缺失~~ | ✅ 已修复：汇聚型边界外生成岛弧 + 海沟地形对 |

#### 2.5d：噪声参数标定与验证 ✅

| 问题 | 改进方向 |
|------|---------|
| ~~fBm 噪声参数凭经验设定~~ | ✅ 已修复：对比真实 DEM（ETOPO1/GEBCO）功率谱标定 amplitude/persistence |
| ~~噪声各向同性，无构造走向~~ | ✅ 已修复：各向异性噪声（沿山脉走向拉伸） |
| ~~缺乏定量质量评估~~ | ✅ 已修复：高程直方图双峰性、粗糙度-尺度关系、流域统计 |

#### 2.5e：标准测试世界 ✅

| 问题 | 改进方向 |
|------|---------|
| ~~修改参数后无法量化影响~~ | ✅ 已修复：标准测试世界（seed=42, num_nodes=32768），每次改动后对比快照 |

> 自动化地形质量检查（CI 集成、噪声功率谱验证等）暂不实施——在地形效果稳定前难以定义有效的定量指标。

**预期产出**（全部完成）：
- ✅ 标准测试世界配置文件（seed=42, num_nodes=32768）
...
| 功能 | 状态 | 说明 |
|------|------|------|
| 能量平衡模型（EBM） | ✅ 已实现 | `climate_physics.py:equilibrium_temperature()` |
| 纬度 + 海拔温度修正 | ✅ 已实现 | 全球均温 15.0 °C（匹配地球） |
| Hadley/Ferrel/Polar 风带 | ✅ 已实现 | `climate_physics.py:hadley_cell_wind()` |
...
| P2 | 世界线 Diff 可视化 | 2 周 | ★★★ |
| P2 | **TODO: 海岸线渲染性能**（2M 像素 compositing ~6s 首帧） | 待调研 | ★★ |
| P3 | AI 顾问模式 | 1–2 周 | ★★ |
| P3 | 实时协作 | 3–4 周 | ★★ |
| P3 | 世界导出包 | 1 周 | ★★ |
...
将"时间"作为一等公民嵌入所有 DAG 层级。详见 [vision.md §8](vision.md#8-时间作为一等公民)。

| 层级 | 时间尺度 | 数据特点 |
|------|---------|---------|
| 天文学 | 10⁹ 年 | 恒星演化曲线（慢变参数） |
| 地质学 | 10⁸ 年 | 板块漂移动画（每步差分快照） |
| 气候学 | 10⁴–10⁶ 年 | 冰期间冰期（预计算采样） |
| 生态学 | 10³–10⁶ 年 | 物种迁移/灭绝（事件驱动） |
| 文明层 | 10²–10³ 年 | 国家兴衰（状态机事件流） |

前端统一时间控件——切换 DAG 层级时，时间轴自动切换到对应的尺度和单位。
