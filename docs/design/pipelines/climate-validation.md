...
空间正确性（逐 cell）是主指标；直方图匹配（Match rate / Group R²）只做粗校验。

| 指标 | 含义 | 阈值 | 理想值 |
|------|------|------|--------|
| Cohen's Kappa | 逐 cell 一致率扣除随机期望（30 类混淆矩阵） | > 0.25 | > 0.5 |
| 空间准确率 | 逐 cell 30 类精确命中率（`--spatial`） | > 30% | > 50% |
| 群组准确率 | 五大群（A-E）精确命中率 | > 50% | > 70% |
| Match rate | 与 Beck 2018 的面积加权匹配率（辅助，无空间信息） | > 50% | — |
| Group R² | 五大气候群（A-E）的面积相关性（辅助，无空间信息） | > 0.7 | — |

> **为什么 Kappa 是主指标**：Match rate / Group R² 是全局直方图重叠，不含空间信息——
> 把雨林整体标在撒哈拉、沙漠整体标在亚马逊，只要各类总量对，也能拿满分，但对架空世界
> 用户毫无意义。空间准确率含空间信息，但被类别不平衡夸大（30 类里瞎猜也有 5–8% 保底
> 命中）。Kappa = (准确率 − 随机期望) / (1 − 随机期望)，扣掉「碰巧蒙对」后的净提升，
> 是三者里最诚实的空间指标。
...
| 沙漠偏湿 / 内陆降水铺得过开 | 湍流扩散 κ 过强，海洋水汽过度扩散到陆地 | 减小 `moisture_diffusivity_m2s`（config 字段，默认 1e6） |
| 内陆过干（雨林塌成沙漠） | κ 过小，水汽到不了内陆 | 增大 κ / 调 Budyko 再循环参数 |
| 极地温度不够冷 | 反照率未考虑冰盖 | 调整纬度梯度 |

### 5.5 站点诊断（单点季节行为）

§5.1–5.3 的 RMSE / R² / Kappa 都是**空间或统计指标**：它们说「雨林、沙漠、苔原各有多少、
放在哪里对不对」，却看不到「北京 7 月该刮南风、该下 180 mm 雨」这种**单点的季节行为**。
一个模型可以雨林、沙漠的空间分布都对，却让夏季风在东亚/南亚根本没建立——Kappa 对此无感。

`scripts/climate/station_diagnostics.py` 补上这个角度。它内置 26 个参考站（按 Köppen 类型分层，
覆盖 Af 到 ET 各主类，偏重季风区），在 CVT 网格上找最近 cell，采样模型逐月温度/降水/风向，
对比硬编码的标准气候态（WMO / CMA / IMD，来源标在脚本 docstring），输出一张「年均温 /
季节振幅 / 年降水 / 7 月风向 / 1 月风向」的逐站偏差表。

它是**诊断锚点而非验收门槛**：改气候引擎后跑一遍，看季风风向、季节振幅、干湿季有没有被
连带破坏。2026-08 的「东亚/南亚季风没建立」「亚马逊 Af 退化」都是它比 Kappa 更早暴露的。

观测数据当前硬编码，后续可扩展为 ERA5 / CRU 打包文件（脚本 docstring 已留 TODO）。
...
1. **zonal mean 参考值**：从 ERA5/GPCP 提取，分辨率为 2° 纬度带。无法捕获经度方向的误差
   （经度结构由逐格 obs 参考补充：`generate_climate_obs.py` → `climate_obs.json`）。
2. **单点校准风险**：§1–5 的全部验证仅针对现代地球——这在校准意义上充分，
   作为泛化验证不充分（见 §7）。
3. **CVT 网格分辨率**：200k cell ≈ 51 km 间距，仍不足以分辨狭促地形驱动的局地气候
   （如狭窄海峡、城市尺度热岛）。
...
- **慢自转云反馈**：Yang et al. (2013) 的次恒星点云稳定效应是高引也是高争议结论——
  ...
  标定域。本引擎无云物理，此项偏差方向由硬编码决定（见 climate-pipeline.md §13
  地球标定常数）。
...
- 高倾角 → 极地增温、赤道降温（年均）
- 快自转 → Hadley 胞变窄（当前 30° 边界为可配置常数，此项待行星化后方可断言，
  见 climate-pipeline.md §13）
