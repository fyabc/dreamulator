...
5. **海岸不对称雨出调制**（CLIM-02 切片 3，求解器内 k_rain 场）：向岸风携带洋面水汽
   ...
   全部 12 个月与年解。
6. **次行星半球对流增强**（潮汐锁定稳态，均分 12 月，:2263）。
7. **冷陷阱**（`_apply_cold_trap`，求解器末端，CLIM-02 切片 2）：饱和钳制 W ≤ W_sat
   保留，但被截断的雨出率 k·(W−W_sat) **不再删除**——沿该格入流边按到达通量比例
   路由回上风暖区雨出（审计 §2.1/§5.2：「excess rains out upwind — 南极海岸而非
   高原腹地」）。实测此前删除量 = earth 预算的 6.8% / nacrea 10.4%（旧注释「~1-2%」
   低估 5 倍）；路由后 ΣA·P = ΣA·E 在冷阱环节精确成立。
8. **年封顶 11000 mm/yr**（地球实测上限，Mawsynram/Cherrapunji）——按月度值等比例
   封顶，保持季节形状。
