...
- **通用框架**：无统一跨行星标准；四类独立实践 = 温度阈值气候态（Wolf 2017 四态 235/250/
  275–315/330 K）、动力 regime（Haqq-Misra 2018 三 regime）、GCM 互比较场景（THAI/CUISINES）、
  地球分类移植（**同行评审论文为零**，唯一应用是硕士论文）。含义：格点级「热量×供需」两轴组合
...
## §5 通用框架：四类独立实践，无统一标准（压缩节）

**结论**：截至 2026-09，不存在统一的、被社区公用的跨行星气候分类标准（没有行星界的
Köppen）。实际存在四类彼此独立的分类实践（详细判据原文与 30 条文献表见
`private/reviews/_research/ucc-taxonomy-frameworks.md`）：
...
组合是文献空位（既是新颖性，也是无先例可对标的风险）。所有已核实的温度阈值都带**标定域**
（随大气成分/水库存/恒星型移动），与仓库「闭合知识论分类 + 标定域声明」纪律同构。
