...
**为什么选栅格而非纯 Voronoi？**
- 栅格编辑直观（画笔工具），GPU 渲染高效（Three.js shader）
...
- **Three.js WebGL**（GPU）渲染地形：2048×1024 像素实时编辑无卡顿
...
> **参考**：[Paradox](https://www.paradoxinteractive.com/) 的 shader-based terrain rendering；[Azgaar](https://github.com/Azgaar/Fantasy-Map-Generator) 的 SVG overlay 系统

- **地形渲染**：Three.js (R3F) PlaneGeometry + 自定义 GLSL shader
  - 海拔→颜色映射（hypsometric tint，参考 GIS 标准色表）
  - 山体阴影（hillshading，模拟西北 45° 光源）
  - 水面深度暗化 + 高光
- **矢量叠加**：SVG 层覆盖在 WebGL Canvas 上
