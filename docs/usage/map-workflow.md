...
浏览器会自动打开。进入目标世界 → 选择行星 → 进入地图编辑器。

### 5.2 编辑器布局

```
┌─────────────────────────────────────────────────────────────────┐
│  顶栏：返回 · 世界名 · 行星选择 · 分支选择 · 只读标记          │
├──────────┬───────────────────────────────┬──────────────────────┤
│          │                               │  单元格详情          │
│  左面板   │        中央地图视图            │  (悬停查看)          │
│          │                               ├──────────────────────┤
│ ·着色模式 │  地形渲染 + 山体阴影           │  鸟瞰图              │
│ ·矢量叠加 │  + SVG 矢量叠加层             │  (视口位置标注)      │
│ ·数据操作 │  (水平无限环绕)               │                      │
│          │                               │  气候信息            │
│          │                               │  · 温度              │
│          │                               │  · 降水              │
...
| **温度** | 蓝（冷）→ 红（热）渐变 | 查看气候分布 |
| **降水** | 黄（干旱）→ 绿（湿润）渐变 | 分析降水模式 |
| **Köppen** | 按 Köppen 气候分类着色 | 识别气候带（热带雨林、沙漠、苔原等） |
...
| 叠加层 | 显示内容 | 颜色编码 |
|--------|----------|----------|
| **Voronoi 网格** | 圆圈显示 cell 中心点，悬停高亮 | 灰色 |
| **板块边界** | 板块交界处的彩色线段 | 🔴 红 = 汇聚型 · 🔵 蓝 = 张裂型 · 🟡 黄 = 转换型 |
| **河流** | 河流路径线条 | 蓝色，线宽按流量等级缩放 |
| **山脊** | 山脊线 | 橙色 |
| **气候带** | Köppen 分类多边形填充 | 按气候类型着色 |
...
| 板块 | 所属构造板块名称和类型 |
| 地壳类型 | oceanic / continental / mixed |
| 温度 | 年均温度（°C） |
| 降水 | 年降水量（mm） |
| 气候类型 | Köppen 分类（如 Cfa = 亚热带湿润气候） |
| 生物群系 | biome 分类（由生态学引擎填充） |
| 河流流量 | 累积流量等级（0 = 无河流） |
