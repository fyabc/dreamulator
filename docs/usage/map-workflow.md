...
图层按学科分组，四种渲染语义：**底图**（不透明，单选）→ **专题**（全表面着色，单选）→
**填充**（多选叠加）→ **特征线**（多选叠加、置顶）。
...
|------|------|
| 板块 | 按板块 ID 调色板着色 |
| 地壳与边界 | 地壳类型 + 汇聚/离散/转换边界 + 热点/裂谷/造山带 |
...
| 图层 | 说明 |
|------|------|
| Whittaker 群系 | 温度-降水二维生物群系分类 |
| 净初级生产力 | NPP 热力图（连续色阶：暖米→深绿） |
| 文明摇篮 | 高驯化潜力区域高亮（大型食草动物/主食作物/役用动物） |

> 气候与生态共用专题槽位——Köppen、群系、NPP、文明摇篮四选一，通过透明度滑块透视底图地形。|
