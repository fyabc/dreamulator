...
│  图层面板 │        中央地图视图            │                      │
│          │                               │  · 未选中: 行星摘要  │
│【地形·海陆】│  地形渲染 + 山体阴影           │    (海陆比例/高程/   │
│  · 地形   │  (CPU 烘焙纹理 +              │     板块数/节点数)   │
│  · 海陆   │   GPU 分层合成显示)            │                      │
│【地质构造】│                               │  · 悬停: 单元格详情  │
│  · 板块   │  + SVG 高亮层                 │    (经纬度/海拔/     │
│  · 边界   │  (hover/select 反馈)          │     地壳/板块/边界)  │
│ 【气候】  │                               │                      │
│  · Köppen │                               │                      │
│          │                               │  · 选中: 多单元格    │
├──────────┴───────────────────────────────┴──────────────────────┤
...
图层面板按学科概念分组。每个图层按渲染语义分四种 kind：
**底图**（不透明画布，全局唯一）→ **专题**（全表面着色，至多一个、可无）→
**填充**（多选叠加）→ **特征线**（多选叠加、永远置顶）。合成顺序固定为
底图 → 专题 → 填充 → 特征线，因此板块/边界可以叠在 Köppen 气候之上。

**【地形·海陆】组（底图槽位，radio 单选，必选其一）：**

| 图层 | 说明 | 用途 |
|------|------|------|
| **地形** | 自适应分层设色（深蓝→浅蓝→绿→黄→棕→白）+ 海岸线 | 整体地貌概览（默认） |
| **海陆** | 二值着色，深色海洋 + 浅色陆地 | 查看海陆比例 |

**【地质构造】组（多选叠加，眼睛开关 + 透明度滑块）：**

| 图层 | 说明 | 用途 |
|------|------|------|
| **板块** | 按构造板块 ID 调色板着色（填充分类） | 查看板块分布和大小 |
| **地壳与边界** | 地壳类型（大陆褐/海洋蓝）+ 边界（汇聚红·离散绿·转换黄）+ 热点/裂谷/造山带（特征线） | 分析板块边界运动 |

**【气候】组（专题槽位，radio 单选，可为"无"）：**

| 图层 | 说明 | 用途 |
|------|------|------|
| **Köppen 气候** | Beck (2018) 标准色气候分类，以 ~85% 不透明度叠在底图上 | 气候带分布（拖动透明度可透视底图地形） |

> **自适应配色**：地形底图的色标根据行星实际高程范围自动计算断点，
> 无需手动调整。海洋从深蓝（最深）到浅蓝（海岸），陆地进行类似渐变。
>
> **性能**：各图层纹理按数据变化烘焙并缓存；拖动透明度滑块仅更新 GPU
> 合成 uniform，不触发重烘。
...
| +30% range | ██ | `#2B8CBE` | 陆架边缘 — C |
| -0.8% (≥80m) | ██ | `#5BA8D0` | 浅海 — C |
| -0.3% (≥30m) | ██ | `#90D0E8` | 近海 — C |
| -0.1% (≥10m) | ██ | `#C8E8F8` | 水缘 — C |
| 海平面 (0m) | ██ | `#A8D8B0` | 浅绿海岸线 (经典 Bartholomew) |
| +0.5% (≥50m) | ██ | `#6B9E40` | 橄榄绿低地 — B |
| +1% (≥100m) | ██ | `#4A8A2E` | 森林深绿 — B |
| +2% range | ██ | `#2F7A3C` | 暗绿丘陵 — B |
| +8% range | ██ | `#A0B040` | 黄绿中山 — B |
| +18% range | ██ | `#B89058` | 棕褐台地 (与海岸区分明显) |
